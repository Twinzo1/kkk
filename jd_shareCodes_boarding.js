// 京东助力码上车
const BASE_BOARDING_URL = 'http://jd.turinglabs.net/api/v1/jd/@type/create/@shareCode'
...
const shareCodeMap = {
  ...
  jdzz: shareCodes.map(it => it.jdzz)
}
...
const boarding = (shareCode, type) => {
  ...
  const url1 = type != "jdzz" ? BASE_BOARDING_URL : BASE_BOARDING_URL1;
  const url2 = url1.replace('@type', type);
  const url = url2.replace('@shareCode', shareCode);
    ...
    $.get(taskurl(url), (err, resp, data) => {
      ...
      try {
        ...
        if (data.code === 200) {
          ...
          if (`${Object.values(data)}`.search("share code existed|Code Is Exists") != -1) {
            const msg = ""
