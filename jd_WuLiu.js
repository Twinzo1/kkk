/*
京东物流,自用,可N个京东账号,IOS软件用户请使用 https://raw.githubusercontent.com/id77/QuantumultX/master/task/jdWuLiu.js
Node.JS专用
更新时间：2020-11-09
...
!(async() => {
  ...
  // 下载最新代码
  await downFile();
  const content = await fs.readFileSync('./jdWuLiu.js', 'utf8')
  for (let i =0; i < cookiesArr.length; i++) {
    ...
    if (cookie) {
      ...
      $.nickName = '';
      await TotalBean();
      console.log(`*****************开始京东账号${$.index} ${$.nickName || $.UserName}京东物流*******************\n`);
...
async function execSign() {
  ...
  try {
    if (notify.SCKEY || notify.BARK_PUSH || notify.DD_BOT_TOKEN || (notify.TG_BOT_TOKEN && notify.TG_USER_ID) || notify.IGOT_PUSH_KEY) {
        await exec(`${process.execPath} ${jdWuLiuPath} >> ${resultPath}`);
      } else {
      // 如果没有提供通知推送，则打印日志
      console.log('没有提供通知推送，则打印脚本执行日志')
      await exec(`${process.execPath} ${jdWuLiuPath}`, { stdio: "inherit" });
    }
    // await exec("node jdWuLiu.js", { stdio: "inherit" });
    ...
    if ($.isNode()) {
      ...
      if (BarkContent) {
        await notify.sendNotify(`京东物流 - 账号${$.index} - ${$.nickName || $.UserName}`, `【签到号 ${$.index}】: ${$.nickName || $.UserName}\n【签到时间】:  ${$.beanSignTime}\n${BarkContent}`);
      }
    }
    //运行完成后，删除下载的文件
    console.log('运行完成后，删除下载的文件\n')
    await deleteFile(path);//删除result.txt
    await deleteFile(jdWuLiuPath);//删除jdWuLiu.js
    console.log(`*****************京东账号${$.index} ${$.nickName || $.UserName}京东物流完成*******************\n`);
  } catch (e) {
...
async function downFile () {
  ...
  await downloadUrl();
  if ($.body) {
    url = 'https://raw.githubusercontent.com/id77/QuantumultX/master/task/jdWuLiu.js';
  // } else {
  //  url = 'https://cdn.jsdelivr.net/gh/NobyDa/Script@master/JD-DailyBonus/JD_DailyBonus.js';
  }
  await download(url, './')
...
async function changeFile (content) {
  ...
  try {
    await fs.writeFileSync(jdWuLiuPath, newContent, 'utf8');
    console.log('替换变量完毕');
  } catch (e) {
    console.log("京东物流写入文件异常:" + e);
  }
}
...
function downloadUrl(url = 'https://raw.githubusercontent.com/id77/QuantumultX/master/task/jdWuLiu.js') {
  return new Promise(resolve => {
    $.get({url}, async (err, resp, data) => {
      try {
        if (err) {
          console.log(`${JSON.stringify(err)}`)
          console.log(`检测到您不能访问外网,将使用CDN下载jdWuLiu.js文件`)
        } else {
          $.body = data;
        }
      } catch (e) {
