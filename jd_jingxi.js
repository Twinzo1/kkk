/*
京喜签到,自用,可N个京东账号,IOS软件用户请使用 https://raw.githubusercontent.com/Sunert/Scripts/master/Task/jingxi.js
Node.JS专用
更新时间：2020-11-09
...
async function downFile () {
  ...
  await downloadUrl();
  if ($.body) {
    url = 'https://raw.githubusercontent.com/Sunert/Scripts/master/Task/jingxi.js';
  // } else {
  //  url = 'https://cdn.jsdelivr.net/gh/NobyDa/Script@master/JD-DailyBonus/JD_DailyBonus.js';
  }
  await download(url, './')
...
function downloadUrl(url = 'https://raw.githubusercontent.com/Sunert/Scripts/master/Task/jingxi.js') {
  return new Promise(resolve => {
    $.get({url}, async (err, resp, data) => {
      try {
        if (err) {
          console.log(`${JSON.stringify(err)}`)
          console.log(`检测到您不能访问外网,将使用CDN下载jingxi.js文件`)
        } else {
          $.body = data;
        }
      } catch (e) {
